...
// MARK: - Nested Resource API (path parameter declared on the base path)

/// A nested resource whose parent identifier lives on the group's base path, as in
/// `/v1/books/:bookId/chats` — the shape that once lost `bookId` during decoding.
@APIGroup(path: "/v1/books/:bookId/chats", auth: .none)
public enum NestedResourceAPI {
    /// Empty sub-path: the only parameter comes from the base path.
...
// The service protocol is generated by the @APIGroup macro.
...
public struct NestedResourceAPIServiceImpl: NestedResourceAPIService {
    ...
    public func handle(_ input: NestedResourceAPI.List, context: ServiceContext) async throws -> [TestItem] {
        // Echo the identifier back so the test can prove it survived decoding.
